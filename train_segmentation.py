...
def train(arguments):
    ...
    valid_dataset = ds_class(template_path, ds_path, studies, split='validation', transform=ds_transform['valid'],
                             train_size=split_opts.train_size, test_size=split_opts.test_size,
                             valid_size=split_opts.validation_size, split_seed=split_opts.seed)
    test_dataset = ds_class(template_path, ds_path, studies, split='test', transform=ds_transform['valid'],
                            train_size=split_opts.train_size, test_size=split_opts.test_size,
                            valid_size=split_opts.validation_size, split_seed=split_opts.seed)
    ...
    while delta_loss > 0.001:
        for epoch in range(model.which_epoch, train_opts.n_epochs):
            ...
            losses_log = pd.read_excel(os.path.join('checkpoints', json_opts.model.experiment_name, 'loss_log.xlsx'))
            if (errors['Seg_Loss'] < losses_log['Seg_Loss'].min()) and (epoch % train_opts.save_epoch_freq == 0):
                model.save(epoch)

            # Update the model learning rate
            model.update_learning_rate()

            delta_loss = prev_loss - errors['Seg_Loss']
            prev_loss = errors['Seg_Loss']
